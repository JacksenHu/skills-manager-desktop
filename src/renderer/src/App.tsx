...
import { FolderOpen, HardDrive, Link2, Moon, RefreshCw, Settings, Sun } from 'lucide-react'
import type { AgentStatus, AppConfig, JunctionState } from '@shared/types'

type Tab = 'connect' | 'skills' | 'updates' | 'settings'
...
const STATE_META: Record<JunctionState, { label: string; cls: string }> = {
  ...
  empty: { label: '已归并', cls: 'bg-amber-500/15 text-amber-400 ring-amber-500/30' },
    ...
    <div className="group rounded-xl border border-slate-200 bg-white p-4 transition hover:border-sky-400/60 dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <div className="truncate font-medium">{agent.key}</div>
      ...
      <div className="mt-3 flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
        <span>{agent.state === 'active' ? `${agent.skillCount} 个技能可见` : '—'}</span>
        <button
